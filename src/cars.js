...
function createMarkup(arr) {
    return arr.map(({id, model, type, price,img}) => `<li id=${id} class="item">
  <img class="img" src="${img}" alt="${model}" />
...
function onClick(evt) {
  ...
  if (evt.target.classList.contains("favourite") && !evt.target.classList.contains("star_active")) {

    evt.target.classList.add("star_active")
    const id = evt.target.closest('li').getAttribute('id');
    const result = cars.find((item) => Number(id) === item.id)

    const { model, type } = result;
    const markup = `<li><p>${model} ${type}</p></li>`
    favouriteList.insertAdjacentHTML("beforeend", markup)
    console.log(choosenCars)
  } else if (evt.target.classList.contains("favourite") && evt.target.classList.contains("star_active")) {
